   ...
   while (Serial.available())
        {  serigelen = Serial.readString();
           serigelen= serigelen.substring(0,8);
            if ( serigelen =="FHALMALM")
              {  if (almkon == 1) yolla("FHALMALM");
                 
               }
           
           if ( serigelen !="FHALMALM")  yolla(serigelen);
...
{  
  ...
  if (wifigelen == "THALMOFF" ) almkon = 2;
  if (wifigelen == "THALMONN" ) almkon = 1;
  if (wifigelen.substring(0,2) =="TH") Serial.println(wifigelen);   

  if (wifigelen == "TAREPORT" ) rap();
  if (wifigelen == "THREPORT" ) rap();
...
void rap()
{
  if ( almkon == 1) yolla ("FHOKOKON");  
  if ( almkon == 2) yolla ("FHOKOKOF");  
