   ...
   while (Serial.available())
        {  serigelen = Serial.readString();
           serigelen= serigelen.substring(0,8);
            if ( serigelen =="FCALMALM")
              {  if (almkon == 1) yolla("FCALMALM");
                 
               }
           
           if ( serigelen !="FCALMALM")  yolla(serigelen);
...
void gelen( String wifigelen)
{
  ...
  if (wifigelen =="TCALMOFF")  almkon = 2;
  if (wifigelen =="TCALMONN")  almkon = 1;
       
if (wifigelen.substring(0,2) =="TC") Serial.println(wifigelen); 
     
if (wifigelen == "TAREPORT" ) rap();
if (wifigelen == "TCREPORT" ) rap();
...
void rap()
{
  if ( almkon == 1) yolla ("FCOKOKON");  
  if ( almkon == 2) yolla ("FCOKOKOF");  
...
/*
 ...
 {  
  ...
  voltaj /= 100; 
  wifigiden= String( voltaj);
  wifigiden = "FCREPVLT" + wifigiden;

  if(voltaj <= 11.6 ) {
   
    if (voltkon ==1 ){ yolla ("FCPOWALM");  
         voltkon=0;
    }
